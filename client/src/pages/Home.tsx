      ...
      <main className="mx-auto grid max-w-7xl gap-14 pt-20 lg:grid-cols-[1.05fr_.95fr] lg:items-center lg:pt-28">
        ...
        <section className="relative min-h-[490px]">
          ...
          <div className="relative z-10 mx-auto mt-8 max-w-[460px] rounded-[32px] border border-white/70 bg-[#fbfaff] p-6 shadow-[0_20px_60px_rgba(67,56,125,.12)]">
            <div className="flex items-center justify-between"><span className="text-sm font-medium text-[#7163f6]">FORMA / TODAY</span><span className="rounded-full bg-[#f1efff] px-3 py-1 text-xs text-[#6559df]">Sunday, 17</span></div>
            <p className="mt-8 font-display text-3xl tracking-[-.04em] text-[#242137]">A future with more room to breathe.</p>
            <div className="relative mt-10 h-48 overflow-hidden rounded-3xl bg-[#26223f] p-5 text-white"><div className="absolute -right-12 -top-12 h-40 w-40 rounded-full bg-[#ffbf7e] opacity-90" /><div className="absolute -bottom-14 left-12 h-36 w-36 rounded-full bg-[#8c7fff]" /><span className="relative text-xs uppercase tracking-[.16em] text-white/55">Dream</span><p className="relative mt-3 max-w-[220px] text-xl font-medium leading-6">Build a life that reflects what matters.</p><div className="absolute bottom-5 left-5 flex items-center gap-2 text-xs text-white/65"><Diamond className="h-3.5 w-3.5" /> Personal vision</div></div>
            <div className="mt-5 flex items-center justify-between rounded-2xl bg-[#f2f0ff] p-4"><div className="flex items-center gap-3"><div className="grid h-9 w-9 place-items-center rounded-xl bg-[#7567f5] text-white"><Target className="h-4 w-4" /></div><div><p className="text-sm font-medium text-[#302b49]">The next meaningful step</p><p className="text-xs text-[#77718f]">45 minutes, protected today</p></div></div><Play className="h-4 w-4 text-[#7567f5]" /></div>
          </div>
        </section>
      </main>
    </div>
...
function CreateDream({ onCreated }: { onCreated: () => void }) {
  ...
  const create = trpc.forma.dreams.create.useMutation({ onSuccess: () => { onCreated(); setOpen(false); setTitle(""); setDescription(""); } });
  const submit = (event: FormEvent) => { event.preventDefault(); create.mutate({ title, description, color: "#7163f6" }); };
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button className="rounded-full bg-[#7163f6] px-5 hover:bg-[#6053e9]"><Plus className="mr-1.5 h-4 w-4" /> New dream</Button></DialogTrigger><DialogContent className="rounded-3xl border-0 p-7 sm:max-w-md"><DialogHeader><DialogTitle className="font-display text-3xl tracking-[-.045em]">Give the future a name.</DialogTitle><DialogDescription>Start with the image of a life, change or experience you want to shape.</DialogDescription></DialogHeader><form onSubmit={submit} className="mt-4 space-y-4"><Input value={title} onChange={e => setTitle(e.target.value)} placeholder="e.g. A slower, stronger year" className="rounded-xl border-[#e6e2ed]" required /><Textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="What does it feel and look like?" className="min-h-28 rounded-xl border-[#e6e2ed]" /><Button disabled={create.isPending} className="w-full rounded-xl bg-[#7163f6] hover:bg-[#6053e9]">{create.isPending ? "Creating…" : "Create dream"}</Button></form></DialogContent></Dialog>;
}
...
function CreateTask({ tasks, actions, milestones, onCreated }: { tasks: any[]; actions: any[]; milestones: any[]; onCreated: () => void }) {
  ...
  const create = trpc.forma.tasks.create.useMutation({ onSuccess: () => { onCreated(); setOpen(false); setTitle(""); } });
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button variant="outline" className="rounded-full border-[#ddd8e8] bg-white px-4 text-[#484258]"><Plus className="mr-1.5 h-4 w-4" /> Add task</Button></DialogTrigger><DialogContent className="max-h-[88vh] overflow-y-auto rounded-3xl border-0 p-7 sm:max-w-lg"><DialogHeader><DialogTitle className="font-display text-3xl tracking-[-.045em]">Name the next action.</DialogTitle><DialogDescription>Make the task specific, place it in context, then protect time for it.</DialogDescription></DialogHeader><form onSubmit={event => { event.preventDefault(); create.mutate({ title, estimateMinutes: Number(estimate), priority, dueAt: dueAt ? new Date(dueAt) : undefined, parentId: parentId ? Number(parentId) : undefined, actionId: actionId ? Number(actionId) : undefined, milestoneId: milestoneId ? Number(milestoneId) : undefined }); }} className="mt-4 grid gap-3"><Input value={title} onChange={e => setTitle(e.target.value)} placeholder="What is the next meaningful step?" required className="rounded-xl border-[#e6e2ed]" /><div className="grid gap-3 sm:grid-cols-2"><Input value={estimate} onChange={e => setEstimate(e.target.value)} type="number" min="15" max="1440" className="rounded-xl border-[#e6e2ed]" /><select value={priority} onChange={event => setPriority(event.target.value as typeof priority)} className="h-10 rounded-xl border border-[#e6e2ed] bg-white px-3 text-sm"><option value="low">Low priority</option><option value="medium">Medium priority</option><option value="high">High priority</option><option value="critical">Critical priority</option></select></div><Input value={dueAt} onChange={event => setDueAt(event.target.value)} type="datetime-local" className="rounded-xl border-[#e6e2ed]" /><div className="grid gap-3 sm:grid-cols-2"><select value={actionId} onChange={event => setActionId(event.target.value)} className="h-10 rounded-xl border border-[#e6e2ed] bg-white px-3 text-sm"><option value="">Link an action (optional)</option>{actions.map(action => <option key={action.id} value={action.id}>{action.title}</option>)}</select><select value={milestoneId} onChange={event => setMilestoneId(event.target.value)} className="h-10 rounded-xl border border-[#e6e2ed] bg-white px-3 text-sm"><option value="">Link a milestone (optional)</option>{milestones.map(milestone => <option key={milestone.id} value={milestone.id}>{milestone.title}</option>)}</select></div><select value={parentId} onChange={event => setParentId(event.target.value)} className="h-10 rounded-xl border border-[#e6e2ed] bg-white px-3 text-sm"><option value="">Independent task</option>{tasks.map(task => <option key={task.id} value={task.id}>Subtask of: {task.title}</option>)}</select><Button disabled={create.isPending} className="mt-1 w-full rounded-xl bg-[#1f1d33] hover:bg-[#312e4c]">{create.isPending ? "Saving…" : "Save task"}</Button></form></DialogContent></Dialog>;
}
...
function ScheduleBlock({ tasks, calendars, onScheduled }: { tasks: any[]; calendars: any[]; onScheduled: () => void }) {
  ...
  const schedule = trpc.forma.calendars.schedule.useMutation({ onSuccess: () => { onScheduled(); setOpen(false); setTitle(""); } });
  ...
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button className="rounded-full bg-[#7163f6] px-4 hover:bg-[#6053e9]"><CalendarDays className="mr-1.5 h-4 w-4" /> Schedule block</Button></DialogTrigger><DialogContent className="rounded-3xl border-0 p-7 sm:max-w-lg"><DialogHeader><DialogTitle className="font-display text-3xl tracking-[-.045em]">Protect the time.</DialogTitle><DialogDescription>Turn a task or action into a visible commitment in your calendar.</DialogDescription></DialogHeader><form onSubmit={event => { event.preventDefault(); schedule.mutate({ title, taskId: taskId ? Number(taskId) : undefined, calendarId: calendarId ? Number(calendarId) : undefined, startsAt: new Date(startsAt), endsAt: new Date(endsAt) }); }} className="mt-5 grid gap-3"><select value={taskId} onChange={event => chooseTask(event.target.value)} className="h-11 rounded-xl border border-[#e5e1eb] bg-white px-3 text-sm"><option value="">Independent block</option>{tasks.map(task => <option key={task.id} value={task.id}>{task.title}</option>)}</select><Input value={title} onChange={event => setTitle(event.target.value)} placeholder="What will this time make room for?" required className="rounded-xl" /><select value={calendarId} onChange={event => setCalendarId(event.target.value)} className="h-11 rounded-xl border border-[#e5e1eb] bg-white px-3 text-sm"><option value="">Personal rhythm (default)</option>{calendars.map(calendar => <option key={calendar.id} value={calendar.id}>{calendar.name}</option>)}</select><div className="grid gap-3 sm:grid-cols-2"><Input value={startsAt} onChange={event => setStartsAt(event.target.value)} type="datetime-local" required className="rounded-xl" /><Input value={endsAt} onChange={event => setEndsAt(event.target.value)} type="datetime-local" required className="rounded-xl" /></div><Button disabled={schedule.isPending} className="mt-1 rounded-xl bg-[#1f1d33] hover:bg-[#312e4c]">{schedule.isPending ? "Scheduling…" : "Schedule block"}</Button></form></DialogContent></Dialog>;
}
...
function CreateCalendar({ onCreated }: { onCreated: () => void }) {
  ...
  const create = trpc.forma.calendars.create.useMutation({ onSuccess: () => { onCreated(); setOpen(false); setName(""); } });
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button variant="outline" className="rounded-full border-[#ded9e8] bg-white px-4 text-[#484258]"><Plus className="mr-1.5 h-4 w-4" /> Calendar</Button></DialogTrigger><DialogContent className="rounded-3xl border-0 p-7 sm:max-w-md"><DialogHeader><DialogTitle className="font-display text-3xl tracking-[-.045em]">Create a life calendar.</DialogTitle><DialogDescription>Separate sport, focus, relationships or any rhythm you want to make visible.</DialogDescription></DialogHeader><form onSubmit={event => { event.preventDefault(); create.mutate({ name, calendarType, color }); }} className="mt-5 space-y-3"><Input value={name} onChange={event => setName(event.target.value)} placeholder="e.g. Deep work" required className="rounded-xl" /><Input value={calendarType} onChange={event => setCalendarType(event.target.value)} placeholder="Type, e.g. discipline" required className="rounded-xl" /><div className="flex items-center gap-3 rounded-xl border border-[#e5e1eb] px-3 py-2"><input value={color} onChange={event => setColor(event.target.value)} type="color" className="h-7 w-9 rounded border-0 bg-transparent p-0" /><span className="text-sm text-[#726c7b]">Choose its visual identity</span></div><Button disabled={create.isPending} className="w-full rounded-xl bg-[#7163f6]">{create.isPending ? "Creating…" : "Create calendar"}</Button></form></DialogContent></Dialog>;
}
...
function ManualTime({ tasks, onSaved }: { tasks: any[]; onSaved: () => void }) {
  ...
  const add = trpc.forma.time.addManual.useMutation({ onSuccess: () => { onSaved(); setOpen(false); } });
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button size="sm" variant="ghost" className="rounded-full text-xs text-[#7568ee] hover:bg-[#f0eeff]"><TimerReset className="mr-1 h-3.5 w-3.5" /> Log time</Button></DialogTrigger><DialogContent className="rounded-3xl border-0 p-7 sm:max-w-md"><DialogHeader><DialogTitle className="font-display text-3xl tracking-[-.045em]">Record the real work.</DialogTitle><DialogDescription>Keep your plan honest by logging time you have already invested.</DialogDescription></DialogHeader><form onSubmit={event => { event.preventDefault(); add.mutate({ taskId: Number(taskId), startedAt: new Date(startedAt), endedAt: new Date(endedAt) }); }} className="mt-5 space-y-3"><select value={taskId} onChange={event => setTaskId(event.target.value)} required className="h-11 w-full rounded-xl border border-[#e5e1eb] bg-white px-3 text-sm"><option value="">Choose a task</option>{tasks.map(task => <option key={task.id} value={task.id}>{task.title}</option>)}</select><Input type="datetime-local" value={startedAt} onChange={event => setStartedAt(event.target.value)} required className="rounded-xl" /><Input type="datetime-local" value={endedAt} onChange={event => setEndedAt(event.target.value)} required className="rounded-xl" /><Button disabled={add.isPending} className="w-full rounded-xl bg-[#1f1d33]">{add.isPending ? "Saving…" : "Save time entry"}</Button></form></DialogContent></Dialog>;
}

function TaskList({ tasks, timeEntries, refresh }: { tasks: any[]; timeEntries: any[]; refresh: () => void }) {
  const update = trpc.forma.tasks.updateStatus.useMutation({ onSuccess: refresh });
  const start = trpc.forma.time.start.useMutation({ onSuccess: refresh });
  const stop = trpc.forma.time.stop.useMutation({ onSuccess: refresh });
  ...
  return <section className="surface-card overflow-hidden"><div className="flex items-center justify-between border-b border-[#efecf3] px-5 py-4"><div><p className="text-sm font-semibold text-[#343044]">Your focused list</p><p className="mt-0.5 text-xs text-[#8a8595]">Small steps, visible momentum</p></div><div className="flex items-center gap-2"><ManualTime tasks={tasks} onSaved={refresh} /><ListChecks className="h-5 w-5 text-[#7163f6]" /></div></div><div className="divide-y divide-[#efecf3]">{tasksToShow.length ? tasksToShow.map(task => { const running = runningTaskIds.has(task.id); const actualMinutes = Math.round(timeEntries.filter(entry => entry.taskId === task.id).reduce((sum, entry) => sum + entry.durationSeconds, 0) / 60); const progress = task.estimateMinutes ? Math.min(100, Math.round((actualMinutes / task.estimateMinutes) * 100)) : 0; return <div key={task.id} className="group flex items-start gap-3 px-5 py-3.5"><button onClick={() => update.mutate({ taskId: task.id, status: task.status === "done" ? "todo" : "done" })} className={cn("mt-1 grid h-5 w-5 shrink-0 place-items-center rounded-full border transition-colors", task.status === "done" ? "border-[#7163f6] bg-[#7163f6] text-white" : "border-[#cfc9dc] text-transparent hover:border-[#7163f6]")}><Check className="h-3 w-3" /></button><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><p className={cn("truncate text-sm font-medium", task.status === "done" ? "text-[#aaa5b1] line-through" : "text-[#3b374b]")}>{task.title}</p>{task.parentId && <span className="rounded-full bg-[#f0eeff] px-1.5 py-0.5 text-[9px] font-semibold text-[#6d61df]">subtask</span>}</div><div className="mt-1 flex items-center gap-2 text-[11px] text-[#938e9d]"><span className={cn("priority-dot", `priority-${task.priority}`)} />{task.priority} · {minutes(task.estimateMinutes)} {task.dueAt && `· ${friendlyDate(task.dueAt)}`}</div><div className="mt-2 flex items-center gap-2"><Progress value={progress} className="h-1.5 flex-1 bg-[#eeebf4] [&>div]:bg-[#7163f6]" /><span className="shrink-0 text-[10px] text-[#8e8898]">{minutes(actualMinutes)} / {minutes(task.estimateMinutes)}</span></div></div><Button onClick={() => running ? stop.mutate({ taskId: task.id }) : start.mutate({ taskId: task.id })} size="icon" variant="ghost" className={cn("mt-0.5 h-8 w-8 rounded-full", running ? "bg-[#fff0ed] text-[#ed735f]" : "text-[#8c8795] hover:bg-[#f0edff] hover:text-[#7163f6]")} aria-label={running ? "Stop timer" : "Start timer"}>{running ? <Pause className="h-3.5 w-3.5 fill-current" /> : <Play className="h-3.5 w-3.5 fill-current" />}</Button></div>; }) : <div className="px-5 py-10 text-center"><Circle className="mx-auto h-7 w-7 text-[#d5d0dc]" /><p className="mt-3 text-sm font-medium text-[#615c6b]">The day is open.</p><p className="mt-1 text-xs text-[#9892a1]">Add one task that deserves your attention.</p></div>}</div></section>;
}

function DreamCards({ dreams }: { dreams: any[] }) {
  return <section className="surface-card overflow-hidden"><div className="flex items-center justify-between border-b border-[#efecf3] px-5 py-4"><div><p className="text-sm font-semibold text-[#343044]">Dreams in focus</p><p className="mt-0.5 text-xs text-[#8a8595]">The direction behind the details</p></div><WandSparkles className="h-5 w-5 text-[#7163f6]" /></div><div className="grid gap-3 p-4">{dreams.length ? dreams.slice(0, 3).map((dream, index) => { const color = colorFromVisual(dream.visualConfig); return <article key={dream.id} className="dream-card" style={{ background: `linear-gradient(135deg, ${color}, #29243f 120%)` }}><div className="relative z-10"><div className="flex items-center justify-between"><span className="rounded-full bg-white/15 px-2.5 py-1 text-[10px] font-medium uppercase tracking-[.13em] text-white/80">{dream.status}</span><Diamond className="h-4 w-4 fill-white/30 text-white/80" /></div><h3 className="mt-9 font-display text-xl tracking-[-.035em] text-white">{dream.title}</h3><p className="mt-2 line-clamp-2 text-xs leading-5 text-white/70">{dream.description || "A direction worth making room for."}</p></div><div className="absolute -bottom-9 -right-8 h-28 w-28 rounded-full bg-white/20 blur-[1px]" />{index === 0 && <div className="absolute right-7 top-9 h-5 w-5 rounded-full border border-white/40 bg-white/15" />}</article>; }) : <div className="rounded-2xl bg-[#f7f5fb] p-7 text-center"><Lightbulb className="mx-auto h-7 w-7 text-[#9e94ee]" /><p className="mt-3 text-sm font-medium text-[#514b61]">Begin with a dream.</p><p className="mt-1 text-xs leading-5 text-[#928c9b]">Your visual north star will keep plans meaningful.</p></div>}</div></section>;
}
...
function GoalStudio({ dreams, goals, roadmaps, milestones, actions, refresh }: { dreams: any[]; goals: any[]; roadmaps: any[]; milestones: any[]; actions: any[]; refresh: () => void }) {
  ...
  const createGoal = trpc.forma.goals.create.useMutation({ onSuccess: refresh });
  const createRoadmap = trpc.forma.roadmaps.create.useMutation({ onSuccess: refresh });
  const createMilestone = trpc.forma.milestones.create.useMutation({ onSuccess: refresh });
  const createAction = trpc.forma.actions.create.useMutation({ onSuccess: refresh });
  ...
  return <section className="surface-card overflow-hidden"><div className="flex flex-col gap-4 border-b border-[#efecf3] p-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-semibold text-[#343044]">Goal & roadmap studio</p><p className="mt-1 text-xs text-[#8a8595]">A dream becomes real when its next stage is clear.</p></div><div className="flex flex-wrap gap-2"><Dialog><DialogTrigger asChild><Button size="sm" className="rounded-full bg-[#7163f6] hover:bg-[#6053e9]"><Plus className="mr-1 h-3.5 w-3.5" /> Goal</Button></DialogTrigger><DialogContent className="rounded-3xl border-0 p-7"><DialogHeader><DialogTitle className="font-display text-3xl">Name the destination.</DialogTitle><DialogDescription>Connect a measurable goal to the dream that gives it meaning.</DialogDescription></DialogHeader><form onSubmit={event => { event.preventDefault(); createGoal.mutate({ dreamId: Number(goalDreamId), title: goalTitle }); setGoalTitle(""); }} className="mt-5 space-y-3"><select value={goalDreamId} onChange={event => setGoalDreamId(event.target.value)} required className="h-11 w-full rounded-xl border border-[#e5e1eb] bg-white px-3 text-sm"><option value="">Choose a dream</option>{dreams.map(dream => <option key={dream.id} value={dream.id}>{dream.title}</option>)}</select><Input value={goalTitle} onChange={event => setGoalTitle(event.target.value)} placeholder="What does success look like?" required className="rounded-xl" /><Button disabled={createGoal.isPending} className="w-full rounded-xl bg-[#7163f6]">Create goal</Button></form></DialogContent></Dialog>
      <Dialog><DialogTrigger asChild><Button size="sm" variant="outline" className="rounded-full border-[#ded9e8] bg-white"><Plus className="mr-1 h-3.5 w-3.5" /> Roadmap</Button></DialogTrigger><DialogContent className="rounded-3xl border-0 p-7"><DialogHeader><DialogTitle className="font-display text-3xl">Plot the path.</DialogTitle><DialogDescription>Give a goal a sequence of meaningful milestones.</DialogDescription></DialogHeader><form onSubmit={event => { event.preventDefault(); createRoadmap.mutate({ goalId: Number(roadmapGoalId), title: roadmapTitle }); setRoadmapTitle(""); }} className="mt-5 space-y-3"><select value={roadmapGoalId} onChange={event => setRoadmapGoalId(event.target.value)} required className="h-11 w-full rounded-xl border border-[#e5e1eb] bg-white px-3 text-sm"><option value="">Choose a goal</option>{goals.map(goal => <option key={goal.id} value={goal.id}>{goal.title}</option>)}</select><Input value={roadmapTitle} onChange={event => setRoadmapTitle(event.target.value)} placeholder="e.g. Foundation sprint" required className="rounded-xl" /><Button disabled={createRoadmap.isPending} className="w-full rounded-xl bg-[#1f1d33]">Create roadmap</Button></form></DialogContent></Dialog></div></div>
    <div className="divide-y divide-[#efecf3]">{goals.length ? goals.map(goal => <article key={goal.id} className="p-5"><div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><span className="grid h-9 w-9 place-items-center rounded-xl bg-[#f0eeff] text-[#7163f6]"><Goal className="h-4 w-4" /></span><div><h3 className="text-sm font-semibold text-[#3b374a]">{goal.title}</h3><p className="mt-0.5 text-xs text-[#908a99]">{goal.status} · {goal.targetDate ? friendlyDate(goal.targetDate) : "No target date yet"}</p></div></div><Badge className="w-fit rounded-full bg-[#edf8f2] text-[#398d6c] hover:bg-[#edf8f2]">linked to dream</Badge></div><div className="mt-4 space-y-3">{roadmapByGoal(goal.id).map(roadmap => <div key={roadmap.id} className="rounded-2xl bg-[#faf9fc] p-4"><div className="flex items-center gap-2"><Layers3 className="h-4 w-4 text-[#6e62ea]" /><p className="text-sm font-medium text-[#514b60]">{roadmap.title}</p></div><div className="mt-3 space-y-2 border-l border-[#dcd7ef] pl-4">{milestonesByRoadmap(roadmap.id).map(milestone => <div key={milestone.id}><div className="flex items-center gap-2 text-sm text-[#625d6c]"><span className="h-2 w-2 rounded-full bg-[#a497ff]" />{milestone.title}</div>{actionsByMilestone(milestone.id).map(action => <div key={action.id} className="ml-4 mt-1 flex items-center gap-2 text-xs text-[#938d9c]"><MoveRight className="h-3 w-3" />{action.title} · {minutes(action.estimateMinutes)}</div>)}</div>)}</div></div>)}</div></article>) : <div className="p-10 text-center"><Target className="mx-auto h-8 w-8 text-[#c4bcf6]" /><p className="mt-3 text-sm font-medium text-[#5d5769]">Choose a dream, then give it a destination.</p><p className="mt-1 text-xs text-[#948e9d]">Your first goal will unlock its roadmap.</p></div>}</div>
    {roadmaps.length > 0 && <div className="grid gap-3 border-t border-[#efecf3] bg-[#fcfbfe] p-4 md:grid-cols-2"><form onSubmit={event => { event.preventDefault(); createMilestone.mutate({ roadmapId: Number(milestoneRoadmapId), title: milestoneTitle }); setMilestoneTitle(""); }} className="flex gap-2"><select value={milestoneRoadmapId} onChange={event => setMilestoneRoadmapId(event.target.value)} required className="min-w-0 flex-1 rounded-xl border border-[#e5e1eb] bg-white px-2 text-xs"><option value="">Roadmap</option>{roadmaps.map(roadmap => <option key={roadmap.id} value={roadmap.id}>{roadmap.title}</option>)}</select><Input value={milestoneTitle} onChange={event => setMilestoneTitle(event.target.value)} placeholder="Add milestone" required className="min-w-0 rounded-xl text-xs" /><Button size="icon" className="shrink-0 rounded-xl bg-[#7163f6]"><Plus className="h-4 w-4" /></Button></form><form onSubmit={event => { event.preventDefault(); createAction.mutate({ milestoneId: Number(actionMilestoneId), title: actionTitle, estimateMinutes: 30 }); setActionTitle(""); }} className="flex gap-2"><select value={actionMilestoneId} onChange={event => setActionMilestoneId(event.target.value)} required className="min-w-0 flex-1 rounded-xl border border-[#e5e1eb] bg-white px-2 text-xs"><option value="">Milestone</option>{milestones.map(milestone => <option key={milestone.id} value={milestone.id}>{milestone.title}</option>)}</select><Input value={actionTitle} onChange={event => setActionTitle(event.target.value)} placeholder="Add action" required className="min-w-0 rounded-xl text-xs" /><Button size="icon" className="shrink-0 rounded-xl bg-[#1f1d33]"><Plus className="h-4 w-4" /></Button></form></div>}
  </section>;
}
...
function CalendarView({ events, calendars, refresh }: { events: any[]; calendars: any[]; refresh: () => void }) {
  ...
  const reschedule = trpc.forma.calendars.reschedule.useMutation({ onSuccess: refresh });
...
function FlowMap({ dreams, goals, roadmaps, milestones, actions, tasks, calendars }: { dreams: any[]; goals: any[]; roadmaps: any[]; milestones: any[]; actions: any[]; tasks: any[]; calendars: any[] }) {
  ...
  return <section className="surface-card overflow-hidden"><div className="flex flex-col gap-4 border-b border-[#efecf3] p-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-semibold text-[#343044]">Flow Map</p><p className="mt-0.5 text-xs text-[#8a8595]">Relationships are calculated from your actual dream, goal and task links.</p></div><div className="flex rounded-full bg-[#f3f1f7] p-1">{(["map", "timeline", "list"] as const).map(item => <button key={item} onClick={() => setMode(item)} className={cn("rounded-full px-3 py-1.5 text-xs capitalize transition-all", mode === item ? "bg-white font-medium text-[#443b9a] shadow-sm" : "text-[#807a8b]")}>{item}</button>)}</div></div>{mode === "map" ? <div className="flow-canvas">{positioned.length ? <><svg className="pointer-events-none absolute inset-0 h-full w-full">{edges.map(edge => <line key={`${edge.from.id}-${edge.to.id}`} x1={`${edge.from.position.x + 12}%`} y1={`${edge.from.position.y + 10}%`} x2={`${edge.to.position.x + 8}%`} y2={`${edge.to.position.y + 10}%`} stroke="#b8b0eb" strokeWidth="2" strokeDasharray="4 4" />)}</svg>{positioned.map(node => <article key={node.id} className={cn("flow-node", `node-${node.size}`)} style={{ "--node-color": node.color, left: `${node.position.x}%`, top: `${node.position.y}%` } as React.CSSProperties}><span className="flow-node-type">{node.type}</span><p>{node.label}</p>{node.type === "Task" && <span className="flow-node-meta">{calendars[0]?.name ?? "Personal"}</span>}</article>)}</> : <div className="grid h-full place-items-center text-center"><MapIcon className="mx-auto h-7 w-7 text-[#b3acf1]" /><p className="mt-3 text-sm text-[#78728a]">Your first connection will appear here.</p></div>}</div> : <div className="p-5">{positioned.length ? positioned.map((node, index) => <div key={node.id} className="flex items-center gap-4 border-b border-[#f0edf3] py-3 last:border-0"><span className="grid h-8 w-8 place-items-center rounded-full text-xs font-semibold text-white" style={{ background: node.color }}>{index + 1}</span><div className="flex-1"><p className="text-sm font-medium text-[#3d394c]">{node.label}</p><p className="mt-0.5 text-xs text-[#918b99]">{node.type} · {node.parentId ? "linked to its parent" : "origin point"}</p></div><MoveRight className="h-4 w-4 text-[#b1abb7]" /></div>) : <p className="py-12 text-center text-sm text-[#9993a2]">Create a dream, goal or task to start mapping.</p>}</div>}</section>;
}

function Assistant({ refresh }: { refresh: () => void }) {
  const [intent, setIntent] = useState("");
  const [proposal, setProposal] = useState<{ id: number; summary: string; commands: PlanCommand[] } | null>(null);
  const propose = trpc.forma.ai.propose.useMutation({ onSuccess: response => { setProposal({ id: response.plan.id, ...response.proposal }); refresh(); } });
  const approve = trpc.forma.ai.approve.useMutation({ onSuccess: () => { refresh(); setProposal(null); } });
  const submit = (event: FormEvent) => { event.preventDefault(); propose.mutate({ intent }); };
  return <section className="assistant-panel overflow-hidden"><div className="relative z-10 max-w-2xl"><div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-white/15 text-[#dcd7ff]"><Sparkles className="h-5 w-5" /></div><p className="mt-7 text-xs font-semibold uppercase tracking-[.16em] text-[#bbb3f2]">Forma intelligence</p><h2 className="mt-3 font-display text-4xl tracking-[-.055em] text-white">Turn a feeling into a plan.</h2><p className="mt-3 max-w-xl text-sm leading-6 text-[#d0cce3]">Describe what you want to move toward. Forma will return a structured, reviewable proposal — nothing changes until you approve it.</p><form onSubmit={submit} className="mt-7 flex flex-col gap-3 sm:flex-row"><Textarea value={intent} onChange={event => setIntent(event.target.value)} placeholder="I want to build a more consistent rhythm around my health…" className="min-h-14 resize-none rounded-2xl border-white/10 bg-white/10 px-4 py-3 text-white placeholder:text-[#aaa4c4] focus-visible:ring-[#b8afff]" required /><Button disabled={propose.isPending} className="h-auto shrink-0 rounded-2xl bg-[#f0edff] px-5 text-[#372e76] hover:bg-white">{propose.isPending ? "Thinking…" : "Create proposal"} <ArrowRight className="ml-2 h-4 w-4" /></Button></form><p className="mt-4 text-[11px] text-[#aaa3ca]">Allowed actions: goals, roadmaps, tasks and calendar suggestions only.</p></div><div className="assistant-glow" />{proposal && <div className="relative z-10 mt-8 rounded-3xl bg-white p-5 text-[#343047] shadow-2xl"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[.13em] text-[#766aec]">Proposal ready for review</p><p className="mt-1 text-sm leading-6 text-[#686277]">{proposal.summary}</p></div><Badge className="w-fit rounded-full bg-[#f0eeff] text-[#6659df] hover:bg-[#f0eeff]">No changes applied</Badge></div><div className="mt-4 space-y-2">{proposal.commands.map((command, index) => <div key={`${command.title}-${index}`} className="flex gap-3 rounded-2xl bg-[#faf9fc] p-3"><span className={cn("grid h-7 w-7 shrink-0 place-items-center rounded-lg text-[10px] font-bold", commandTone[command.command])}>{index + 1}</span><div><p className="text-sm font-medium">{command.title}</p><p className="mt-0.5 text-xs leading-5 text-[#858090]">{command.description}</p><span className="mt-1.5 inline-block text-[10px] font-medium uppercase tracking-[.1em] text-[#8d86a7]">{command.command}</span></div></div>)}</div><div className="mt-5 flex flex-wrap gap-2"><Button onClick={() => approve.mutate({ aiPlanId: proposal.id, idempotencyKey: randomKey() })} disabled={approve.isPending} className="rounded-xl bg-[#7163f6] hover:bg-[#6053e9]"><Check className="mr-1.5 h-4 w-4" />{approve.isPending ? "Applying…" : "Approve and apply"}</Button><Button onClick={() => setProposal(null)} variant="outline" className="rounded-xl border-[#e5e1ec]">Keep planning</Button></div></div>}</section>;
}
...
function Dashboard() {
  ...
  const overview = trpc.forma.overview.useQuery();
  ...
  return <DashboardLayout><div className="mx-auto max-w-[1480px] space-y-7"><WorkspaceHeader view={currentView} workspaceName={data.workspace.name} unread={unread} />
    {currentView === "today" && <><section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Stat icon={Target} label="Goals in motion" value={String(data.goals.filter(goal => goal.status === "active").length)} detail="Connected to your direction" accent="bg-[#f0eeff] text-[#7163f6]" /><Stat icon={CheckCircle2} label="Tasks complete" value={`${completed}/${data.tasks.length}`} detail={data.tasks.length ? "A little progress compounds" : "Start with one clear step"} accent="bg-[#e9f8f1] text-[#319d71]" /><Stat icon={Clock3} label="Time invested" value={minutes(actual)} detail={`of ${minutes(estimate)} estimated`} accent="bg-[#fff3e9] text-[#e6893a]" /><Stat icon={Flame} label="Current focus" value={activeEntry ? "Running" : "Ready"} detail={activeEntry ? "Timer is protecting your attention" : "Choose your next block"} accent="bg-[#fff0ed] text-[#e36f5e]" /></section><section className="grid gap-5 xl:grid-cols-[1.25fr_.75fr]"><div className="space-y-5"><CalendarView events={data.events} calendars={data.calendars} refresh={refresh} /><TaskList tasks={data.tasks} timeEntries={data.timeEntries} refresh={refresh} /></div><div className="space-y-5"><div className="flex items-center justify-between"><div><p className="font-display text-2xl tracking-[-.04em] text-[#302c42]">North star</p><p className="mt-1 text-xs text-[#938d9c]">A reminder of where all this is going.</p></div><CreateDream onCreated={refresh} /></div><DreamCards dreams={data.dreams} /></div></section><section className="grid gap-5 xl:grid-cols-[.8fr_1.2fr]"><Assistant refresh={refresh} /><FlowMap dreams={data.dreams} goals={data.goals} roadmaps={data.roadmaps} milestones={data.milestones} actions={data.actions} tasks={data.tasks} calendars={data.calendars} /></section></>}
    {currentView === "dreams" && <section className="space-y-5"><div className="flex justify-end"><CreateDream onCreated={refresh} /></div><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{data.dreams.map(dream => <article key={dream.id} className="dream-card min-h-64" style={{ background: `linear-gradient(135deg, ${colorFromVisual(dream.visualConfig)}, #29243f 120%)` }}><div className="relative z-10"><Badge className="rounded-full bg-white/15 text-white hover:bg-white/15">{dream.status}</Badge><h2 className="mt-14 font-display text-3xl tracking-[-.045em] text-white">{dream.title}</h2><p className="mt-3 max-w-sm text-sm leading-6 text-white/70">{dream.description || "A meaningful direction waiting for its first step."}</p></div><div className="absolute bottom-5 left-5 flex items-center gap-2 text-xs text-white/70"><Diamond className="h-4 w-4" /> Forma vision</div></article>)}{!data.dreams.length && <div className="col-span-full rounded-3xl border border-dashed border-[#ded9e8] bg-white/60 py-20 text-center"><Lightbulb className="mx-auto h-9 w-9 text-[#9a8eee]" /><h3 className="mt-4 font-display text-2xl text-[#423c52]">What future do you want to make room for?</h3><p className="mx-auto mt-2 max-w-md text-sm text-[#8b8595]">Create a dream, then connect it to a goal and a plan you can act on today.</p></div>}</div><GoalStudio dreams={data.dreams} goals={data.goals} roadmaps={data.roadmaps} milestones={data.milestones} actions={data.actions} refresh={refresh} /></section>}
    ...
    {currentView === "assistant" && <section className="space-y-5"><Assistant refresh={refresh} /><section className="surface-card p-5"><div className="flex items-center justify-between"><div><p className="text-sm font-semibold text-[#353044]">Notifications</p><p className="mt-1 text-xs text-[#8d8797]">AI approval prompts and calendar suggestions arrive here.</p></div><Bell className="h-5 w-5 text-[#7163f6]" /></div><div className="mt-5 space-y-2">{data.notifications.length ? data.notifications.slice(0, 5).map(notification => <article key={notification.id} className="rounded-2xl bg-[#faf9fc] p-3"><p className="text-sm font-medium text-[#474153]">{notification.title}</p><p className="mt-1 text-xs leading-5 text-[#898393]">{notification.body}</p></article>) : <p className="py-7 text-center text-sm text-[#9993a2]">Forma will surface planning prompts here.</p>}</div></section></section>}
  </div></DashboardLayout>;
}
